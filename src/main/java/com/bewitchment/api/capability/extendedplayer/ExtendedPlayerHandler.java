		...
		if (!event.player.world.isRemote && event.phase == TickEvent.Phase.END) {
			...
			if (cap.fortune != null) {
				...
				if (cap.fortuneTime == 0) {
					if (cap.fortune.apply(event.player)) cap.fortune = null;
						...
						cap.fortuneTime = (event.player.getRNG().nextInt(cap.fortune.maxTime - cap.fortune.minTime) + cap.fortune.minTime);
			...
			if (cap.curses != null) {  //check "curse conditions"
				...
				for (Curse curse : curses) {
					if (curse.getCurseCondition() == Curse.CurseCondition.EXIST && event.player.getRNG().nextDouble() < curse.chance)
