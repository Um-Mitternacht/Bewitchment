...
public class RitualWrapper implements IRecipeWrapper {
	...
	private void color(GlyphType gt, Minecraft minecraft) {
		...
		case NORMAL:
			...
			break;
		case ANY:
			colorRandom(minecraft.world.getTotalWorldTime());
			break;
		}
	}
	
	private void colorRandom(long v) {
		int r = (int) (v % 60 / 20);
		switch (r) {
			case 1:
				color(GlyphType.NORMAL, null);
				break;
			case 2:
				color(GlyphType.ENDER, null);
				break;
			case 0:
				color(GlyphType.NETHER, null);
				break;
			default:
				break;
		}
