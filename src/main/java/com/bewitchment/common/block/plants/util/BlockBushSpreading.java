...
public class BlockBushSpreading extends BlockBush {
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (rand.nextInt(35) == 0 && state.getValue(TIMES_SPREAD) < TIMES_SPREAD.getAllowedValues().size() - 1) {
            ...
            if (checkSurrounding(world, pos) && canSustainBush(world.getBlockState(pos0.down())) && world.isAirBlock(pos0))
                world.setBlockState(pos0, rand.nextInt(10) == 0 ? world.getBlockState(pos) : world.getBlockState(pos).cycleProperty(TIMES_SPREAD));
        }
    }
