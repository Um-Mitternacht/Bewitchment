		...
		if (!world.isRemote) {
			if (player.getHeldItem(hand).getItem() instanceof ItemTaglock) {
				...
				if (cap.fortune == null) {
					...
					player.sendStatusMessage(new TextComponentTranslation("fortune.has_fortune", player.getDisplayName(), new TextComponentTranslation("fortune." + cap.fortune.getRegistryName().toString().replace(":", ".")).getFormattedText()), true);
			} else player.sendStatusMessage(new TextComponentTranslation("altar.no_power"), true);
		}
		return true;
	}
