	...
	public void update() {
		if (!world.isRemote && ritual != null) {
			for (BlockPos pos0 : new BlockPos[]{pos, effectivePos})
				if (world.rand.nextInt(16) == 0) Bewitchment.network.sendToDimension(new SpawnParticle(EnumParticleTypes.END_ROD, pos0.getX() + 0.5, pos0.getY() + 0.5, pos0.getZ() + 0.5), effectiveDim);
			if (caster != null) {
