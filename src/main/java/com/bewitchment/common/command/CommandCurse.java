...
public class CommandCurse extends CommandBase {
		...
		if (args.length > 1) {
			if (args[1].equals("get")) {
				if (args.length == 2) {
					...
					if (!cursesStrings.isEmpty())
						sender.sendMessage(new TextComponentTranslation("commands.curse.get", player.getDisplayNameString(), cursesStrings.toString()));
			...
			if (args[1].equals("remove")) {
				if (args.length == 3) {
					...
					if (!player.getCapability(ExtendedPlayer.CAPABILITY, null).hasCurse(curse))
						throw new CommandException("commands.curse.nocurse", player.getDisplayNameString(), args[2]);
					else {
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
		...
		if (args.length == 2)
			return getListOfStringsMatchingLastWord(args, Arrays.asList("get", "add", "remove", "clear"));
