...
public class RitualConjurationHellhoundAlpha extends RitualImpl {

	public RitualConjurationHellhoundAlpha(ResourceLocation registryName, NonNullList<Ingredient> input, NonNullList<ItemStack> output, int timeInTicks, int circles, int altarStartingPower, int powerPerTick) {
	...
	public void onFinish(EntityPlayer player, TileEntity tile, World world, BlockPos pos, NBTTagCompound data, BlockPos effectivePosition, int covenSize) {
		if (!world.isRemote) {
			EntityHellhoundAlpha hellhound = new EntityHellhoundAlpha(world);
			hellhound.setLocationAndAngles(effectivePosition.getX(), effectivePosition.getY(), effectivePosition.getZ(), (float) (Math.random() * 360), 0);
			hellhound.onInitialSpawn(world.getDifficultyForLocation(new BlockPos(hellhound)), (IEntityLivingData) null);
			world.spawnEntity(hellhound);
			if (Math.random() < 0.1) {
				hellhound.addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 6000, 2, false, false));
			}
		}
	}
