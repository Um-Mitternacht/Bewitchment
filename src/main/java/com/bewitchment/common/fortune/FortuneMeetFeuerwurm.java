...
public class FortuneMeetFeuerwurm extends Fortune {
	...
	public boolean apply(EntityPlayer player) {
		...
		if (player.world.isAirBlock(pos) && player.world.isAirBlock(pos.up()) && player.world.getBlockState(pos.down()).canEntitySpawn(feuerwurm)) {
			...
			if (player.getRNG().nextInt(10) < player.world.getDifficulty().ordinal()) feuerwurm.addPotionEffect(new PotionEffect(MobEffects.RESISTANCE, 900, 1));
			if (player.getRNG().nextInt(10) < player.world.getDifficulty().ordinal()) feuerwurm.addPotionEffect(new PotionEffect(MobEffects.STRENGTH, 900, 1));
			if (player.getRNG().nextInt(10) < player.world.getDifficulty().ordinal()) feuerwurm.addPotionEffect(new PotionEffect(MobEffects.SPEED, 900, 1));
			if (player.getRNG().nextInt(10) < player.world.getDifficulty().ordinal()) feuerwurm.addPotionEffect(new PotionEffect(ModPotions.magic_resistance, 900, 1));
			return true;
		}
		return false;
	}
