...
public final class CauldronRegistry {
	...
	private static final Map<Item, ItemValidator<IBrewModifier>> BREW_MODIFIERS = new HashMap<>();
	...
	public static void registerItemModifier(ItemStack stack, IBrewModifier modifier, boolean strict) {
		Item item = stack.getItem();
		if (BREW_MODIFIERS.containsKey(item)) {
			BREW_MODIFIERS.get(item).add(stack, modifier, strict);
		} else {
			BREW_MODIFIERS.put(item, new ItemValidator<IBrewModifier>().add(stack, modifier, strict));
		}
	}
	...
	public static Map<Item, ItemValidator<IBrewModifier>> getBrewModifiers() {
