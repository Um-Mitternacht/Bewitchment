...
public class ItemHorseshoe extends ModItemBauble {
	...
	public void onHurt(LivingHurtEvent event) {
		if (Util.hasBauble(event.getEntityLiving(), this) && (event.getSource().getTrueSource() instanceof EntityLivingBase && BewitchmentAPI.COLD_IRON_WEAKNESS.contains((EntityLivingBase) event.getSource().getTrueSource())) && event.getEntityLiving() instanceof EntityPlayer && MagicPower.attemptDrain(null, (EntityPlayer) event.getEntityLiving(), 20))
