	...
	public boolean isValid(World world, BlockPos pos, EntityPlayer caster, ItemStackHandler inventory) {
		if (world.getTileEntity(pos) instanceof TileEntityGlyph) {
			for (int i = 0; i < inventory.getSlots(); i++) {
				...
				if (stack.getItem() instanceof ItemTaglock && stack.hasTagCompound()) for (Entity entity : world.loadedEntityList)
					if (entity.getPersistentID().equals(UUID.fromString(stack.getTagCompound().getString("boundId")))) return true;
			}
		}
		return false;
	}
